...
subjects=dir('/home/auguser2016/Projects/0001_Chiasm')

% Analyzed models
all_models={'StickZeppelinBall_Model','LiFE_Model'}

% Type of input tracks
all_tracks={'CSD_seed';'CSD_selected';'DT_seed';'DT_selected'}

% We will not perform LiFE filtering here
filtering=false
...
for xx=1:length(all_models)
    for yy=1:length(all_tracks)
        ...
        for i=5:size(subjects,1)%i=5 failed for ce04
            ...
            specific_data=strcat('/home/auguser2016/Projects/Leicester_vs_LiFE/COMMIT/',model,'/',track,'/',subj);
            ...
            fgFileName = strcat(specific_data,'/',subj,'_', track, '_', model,'_streamlines.tck')

            % The final connectome and data astructure will be saved with this name:
            feFileName    = strcat(subj,'_postCOMMIT_chiasm');
            ...
            system(sprintf('source /home/auguser2016/Projects/Leicester_vs_LiFE/COMMIT/07_connecting_rois.sh %s %s %s', subj, strcat(pwd,'/',subj,'_', track_name, '_chiasm.tck'), output_results_folder))  
        ...
        save(strcat('/home/auguser2016/Projects/Leicester_vs_LiFE/COMMIT/COMMIT_', model,'_',track),'con_ips')
